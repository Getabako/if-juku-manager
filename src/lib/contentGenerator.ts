...
export class ContentGenerator {
   ...
   * 自動実行時は過去の投稿履歴を参照して重複を避ける
   */
  async generateContent(request: ContentRequest): Promise<GeneratedContent> {
    const { category, photos, isAutoGenerated } = request;
    ...
    let historySummary = '';
    if (isAutoGenerated) {
      historySummary = await postHistory.getHistorySummaryForGeneration(category);
      logger.info('過去の投稿履歴を参照中...');
    }

    try {
      // 1. トレンドリサーチ
      let trend = await trendResearcher.researchTrend(category);
      ...
      if (isAutoGenerated) {
        ...
        while (await postHistory.isTopicUsed(trend.topic, category, 30) && retryCount < maxRetries) {
          logger.warn(`トピック "${trend.topic}" は過去30日以内に使用済み。別のトピックを取得中...`);
          retryCount++;
          trend = await trendResearcher.researchTrend(category);
        }

        if (retryCount >= maxRetries) {
          logger.warn('新しいトピックの取得に失敗。フォールバックコンテンツを使用します。');
        }
      }

      // 3. 詳細コンテンツ生成（履歴情報を含める）
      const detailedContent = await this.generateDetailedContentWithHistory(
        category,
        trend,
        historySummary
      );
   ...
   * 履歴情報を含めて詳細コンテンツを生成
   */
  private async generateDetailedContentWithHistory(
    category: CategoryType,
    trend: TrendInfo,
...
【出力形式（JSON）】
{
  ...
  "imagePrompts": ["表紙用", "内容1用", "内容2用", "内容3用"]
    ...
    try {
      ...
      logger.warn('履歴考慮コンテンツ生成に失敗、通常生成にフォールバック');
      return trendResearcher.generateDetailedContent(category, trend);
    }
  }
