/**
 * 日々の活動系Instagram投稿ワークフロー
 *
 * Gemini APIとFacebookプロフィールデータを使用して
 * 高崎さんの文体を真似た活動報告投稿を生成・投稿
 */
...
// 投稿履歴ファイル
const HISTORY_PATH = path.join(__dirname, '../../data/activity-post-history.json');

// 活動系トピックリスト
const ACTIVITY_TOPICS = [
  'if塾での授業風景や生徒の成長',
  '秋田クラーク高等学院でのIT教育活動',
  'Xroid Academyのフォローアップセッション',
  '朝のライブ配信の告知・振り返り',
  'AIを活用した業務効率化の日常',
  '子育てと仕事の両立について',
  '秋田の地元イベントや風景',
...
function selectTopic(history: PostHistory): string {
  // 直近5件で使用したトピックを除外
  const recentTopics = history.posts.slice(-5).map(p => p.topic);
  const availableTopics = ACTIVITY_TOPICS.filter(t => !recentTopics.includes(t));

  if (availableTopics.length === 0) {
    // 全て使用済みの場合はランダムに選択
    return ACTIVITY_TOPICS[Math.floor(Math.random() * ACTIVITY_TOPICS.length)];
  }
 ...
 * 活動系投稿ワークフローを実行
 */
async function runActivityPostWorkflow(): Promise<void> {
  logger.info('========================================');
  logger.info('活動系投稿ワークフローを開始');
